...
require_once( FRAMEWORK_PATH . "extends/ExporterMan/entitys/GroupBuyPickupEntity.php" );


/**
 * Class FareCollectionTest
 */
class GroupBuyPickupEntityTest extends DbHero_TestCase {

    private function getTables(){
        return array("consumer_user","wholesale_product","wholesale_product_spec","groupbuying_activity","unified_order","order_has_spec","consumer_user_has_order");
    }

    private function getDirName(){
        return "GroupBuyPickupEntity";
    }
    ...
    public function getDataSet() {
        ...
        foreach ($this->getTables() as $key => $tableName) {
            $dataSet->addTable($tableName,         dirname(__FILE__)."/".$this->getDirName()."/".$tableName.".csv");    
        }
        return $dataSet;
    }

    /**
     * Test groupBuyPickupEntityProvider::preparePushDataWaitting that's excel export can get.
     * @dataProvider groupBuyPickupEntityProvider
     */
    public function testGroupBuyPickupEntity($describe, $params, $expected) {
        echo "$describe \n\r ";
        $entity = new GroupBuyPickupEntity();
        // $ids = $entity->checkActivitysStateByActiveIds($params);
        // var_dump($ids);
        // $result = $entity->getOrderRecordsByActivityIds($ids);
        // var_dump($result);
    ...
    public function groupBuyPickupEntityProvider() {
        return array(
            array(
                //Describe
                "Test Group Buying Pickup Entity Prepare data output.",
                // params
                array(2),
                // expected
                json_decode('  [{
