...
class NewebServiceProvider implements CashFlowServiceProvider {
	...
	public function __construct(NewebServiceConfig $config=null) {
        if(is_null($config)) {
            $this->config = new GeneralNewebServiceConfig();
        }
        else {
            $this->config = $config;
        }
	...
	public function createPaymentParams($orderInfo) {
		...
		return array(			
			...
			'ApproveFlag' 		=> $this->config->APPROVE_FLAG(),
			'DepositFlag' 		=> $this->config->DEPOSIT_FLAG(),
			'Englishmode' 		=> $this->config->ENGLISH_MODE(),
			'iphonepage' 		=> $this->config->IPHONE_PAGE_FLAG(),
			'OrderURL' 			=> $this->config->NOTIFY_URL(),
