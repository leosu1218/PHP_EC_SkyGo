...
/**
 * Class SearchOrderCreateStart
 * Append order's search condition of create datetime.
 */
class SearchOrderCreateStart extends ConditionStatement {
    /**
    ...
    public function append(DbHero &$dao, &$params, &$conditions, &$select, &$search) {
        if(array_key_exists('orderDateStart', $search)) {
            array_push($conditions, '(DATE_FORMAT(uo.create_datetime,"%Y-%m-%d")>=:orderDateStart)');
            $params[':orderDateStart'] = $search['orderDateStart'];
        }
    }
}
?>