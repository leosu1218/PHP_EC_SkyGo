...
/**
 * Class SearchOrderDeliveryDateStart
 * Append order's search condition of create datetime.
 */
class SearchOrderDeliveryDateStart extends ConditionStatement {
    /**
    ...
    public function append(DbHero &$dao, &$params, &$conditions, &$select, &$search) {
        if(array_key_exists('deliveryDateStart', $search)) {
            array_push($conditions, '(DATE_FORMAT(uo.delivery_datetime,"%Y-%m-%d")>=:deliveryDateStart)');
            $params[':deliveryDateStart'] = $search['deliveryDateStart'];
        }
    }
}
?>