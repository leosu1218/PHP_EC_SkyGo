...
/**
 * Class SearchPayDateTimeEnd
 * Append order's search condition of create datetime.
 */
class SearchPayDateTimeEnd extends ConditionStatement {
    /**
    ...
    public function append(DbHero &$dao, &$params, &$conditions, &$select, &$search) {
        if(array_key_exists('payDateTimeEnd', $search)) {
            array_push($conditions, '(DATE_FORMAT(uo.pay_notify_datetime,"%Y-%m-%d")<=:payDateTimeEnd)');
            $params[':payDateTimeEnd'] = $search['payDateTimeEnd'];
        }
    }
}
?>