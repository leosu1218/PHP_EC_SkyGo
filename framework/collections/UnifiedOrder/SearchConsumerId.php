 ...
 */
class SearchConsumerId extends ConditionStatement {
    /**
    ...
    public function append(DbHero &$dao, &$params, &$conditions, &$select, &$search) {
        if(array_key_exists('consumerId', $search) ) {
            array_push($conditions, 'uo.consumer_user_id=:consumer_user_id');
            $params[':consumer_user_id'] = $search['consumerId'];
        }
