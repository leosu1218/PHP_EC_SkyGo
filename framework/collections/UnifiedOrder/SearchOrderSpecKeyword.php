...
/**
 * Class SearchOrderSpecKeyword
 */
class SearchOrderSpecKeyword extends ConditionStatement {
    /**
    ...
    public function append(DbHero &$dao, &$params, &$conditions, &$select, &$search) {
        if(array_key_exists('keyword', $search)) {
            ...
            array_push($conditions, '(wps.serial like :word or wps.name like :word or wsp.name like :word)');
            $params[':word'] = "%$keyword%";
        }
    }
}
