...
class SearchReturnedKeyword extends ConditionStatement {
    ...
    public function append(DbHero &$dao, &$params, &$conditions, &$select, &$search) {
        if(array_key_exists('keyword', $search)) {
            ...
            array_push($conditions, '(uo.buyer_name like :word or ur.delivery_channel like :word or ur.delivery_number like :word or uo.serial like :word or uo.buyer_email like :word)');
            $params[':word'] = "%$keyword%";
        }
    }
}
