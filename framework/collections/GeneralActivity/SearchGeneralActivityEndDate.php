...
/**
 * Class SearchGeneralActivityEndDate
 */
class SearchGeneralActivityEndDate extends ConditionStatement {
    /**
    ...
    public function append(DbHero &$dao, &$params, &$conditions, &$select, &$search) {
        if(array_key_exists('endDateOpen', $search)&&array_key_exists('endDateClose', $search)) {
            ...
            array_push($conditions, '(act.end_date >= :endOpen and act.end_date <= :endClose)');
        }
    }
}
?>