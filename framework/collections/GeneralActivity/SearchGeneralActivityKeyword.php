...
/**
 * Class SearchGeneralActivityKeyword
 */
class SearchGeneralActivityKeyword extends ConditionStatement {
    /**
    ...
    public function append(DbHero &$dao, &$params, &$conditions, &$select, &$search) {
        if(array_key_exists('keyword', $search)) {
            ...
            array_push($conditions, '(act.name like :word or master.name like :word or act.note like :word or wp.tag like :word)');
            $params[':word'] = "%$keyword%";
        }
    }
}
