...
class PlatformUser extends PermissionDbModel implements PermissionDbActor {
	...
	private function DestoryByGroupId($id){
		$userId = $this->getAttribute("id");
		$platformUserHasGroupPermission = new PlatformUserHasGroupPermissionCollection( $this->dao );
		$result = $platformUserHasGroupPermission->getRecords( array("platform_user_id"=>$userId), 1, 9999 );
		$ids = $this->getIdsByRecords($result['records']);
		$effectRows = $platformUserHasGroupPermission->multipleDestroyById($ids);
		if( $effectRows != count($ids) ){
			throw new Exception("Destory PlatformUser has old group permission is error.", 1);
		}
	}

	/**
	*	Move person from old group to new group.
 	*	
	*	@param $newGroupId 	int Attributes want to this group has those permission set ids.
	*	@return boolean true:success, false:error.
	*/
	public function movePersonGroupByNewGroup( $newGroupId ){
		//delete all user group has group
		$oldGroupId = $this->getAttribute("group_id");
		$this->DestoryByGroupId($oldGroupId);

		//append all new group has permissions by newGroup id to mapping group has permission set.
		$collection = new PlatformUserGroupHasPermissionCollection($this->dao);
		$ids = $collection->getPermissionIdsByGroupId( $newGroupId );
		$PUhasGP = new PlatformUserHasGroupPermissionCollection($this->dao);
		return $PUhasGP->append($this->id, $ids);
	}
	...
	*/
	public function appendGroupPermissions($permissions=array()) {		
		if($this->hasPermission("append_group_permission")) {
			$params = array();
	...
	/**
	*	Destory person permissions include PlatformUserHasPermissionSet Collection and PlatformUserHasPermission Collection.
	*
	*/
	public function DestoryPersonPermissions(){
		$userId = $this->id;
		$platformUserGroupPermissionSet = new PlatformUserHasPermissionSetCollection( $this->dao );
		$result = $platformUserGroupPermissionSet->getRecords( array("puid"=>$userId), 1, 9999 );
		$ids = $this->getIdsByRecords($result['records']);
		$effectRows = $platformUserGroupPermissionSet->multipleDestroyById($ids);
		if( $effectRows != count($ids) ){
			throw new Exception("Destory PlatformUser has old group permission set is error.", 1);
		}

		$platformUserGroupPermission = new PlatformUserHasPermissionCollection( $this->dao );
		$result = $platformUserGroupPermission->getRecords( array("platform_user_id"=>$userId), 1, 9999 );
		$ids = $this->getIdsByRecords($result['records']);
		$effectRows = $platformUserGroupPermission->multipleDestroyById($ids);
		if( $effectRows != count($ids) ){
			throw new Exception("Destory PlatformUser has old group permission is error.", 1);
		}
	}
	...
	*/
	public function appendPersonPermissions($permissions=array()) {
		if($this->hasPermission("append_permission")) {
			...
			$collection = new PlatformUserHasPermissionCollection($this->dao);
