...
/**
 * Class StartedGeneralActivityState
 */
class StartedGeneralActivityState extends GeneralActivityState {
    ...
    public function isMatch($record=array()) {
        if(array_key_exists("state", $record) &&
            ...
            if($record["state"] == $this->getStateCode() &&
                ...
                $validator->requireDateGreaterThan($record, "end_date", $now, "date", false)) {
                return true;
            }
        }
        return false;
