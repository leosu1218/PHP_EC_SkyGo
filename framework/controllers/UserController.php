 ...
 */
  public function updateSelf(){
     ...
     try {
        ...
        $loginData = $collection->login($domain, $account, $password);
  ...
  public function updataPassword(){
      ...
      $salt = $collection->generateSalt();
      $hash = $collection->hash($password , $salt);
      $model = $collection->getById($this->params("id"));
      $count = $model->update( array("hash"=>$hash,"salt"=>$salt) );
      if($count != 1) {
          throw new DbOperationException("update Password record to DB fail.");
      }
      return array();
  }
	...
	*	}   	
    ...
    try {
      ...
      //group
      $groupPermissionIds = (new PlatformUserGroupHasPermissionCollection())->getIdsByGroupId($groupId);
      $rowCount   = $user->appendGroupPermissions($groupPermissionIds);

      //person
      $userId = $user->getAttribute("id");
      $personPermissionSetIds = $this->receiver['personPermissions'];
      $PUhasPS = new PlatformUserHasPermissionSetCollection();
      $rowCount = $PUhasPS->append( $userId, $personPermissionSetIds );

      $PShasPermission = new PermissionSetHasPermissionCollection();
      $personPermissions = $PShasPermission->getPermissionIdsByIds($personPermissionSetIds);
      $rowCount 	= $user->appendPersonPermissions($personPermissions);
 	...
 	/**
 	*	GET: 	/user/<userType:\w+>/lsit/<pageNo:\d+>/<pageSize:\d+>
	...
	*	}   	
 	*
 	*	@param $userType string The user's type that want to get record.
 	*	@param $pageNo int 		The record page's number.
 	*	@param $pageSize int 	The record page's size.    	
 	*/
 	public function getList($userType, $pageNo, $pageSize) {
 		$data = array();
  	try {      		      				
  		$actor 		= PlatformUser::instanceBySession();  			
  		$collection = new PlatformUserCollection();
  		...
  		$collection->setActor($actor);  			
  		$records 	= $collection->getRecordsWithGroup(array(), $pageNo, $pageSize);
  ...
  /**
  * GET:   /user/<userType:\w+>/permission/<userId:\d+>
  *   User register API, 
  * Response json: 
  * {
  * }     
  ...
  *                             'password' => <user password> )
  */
  public function getPermissionByUserId($userType,$userId){
    $PUhasPS = new PlatformUserHasPermissionSetCollection();
    $ids = $PUhasPS->getPermissionSetIdsByPlatformUserId($userId);
    $userCollection = new PlatformUserCollection();
    $user = $userCollection->getById($userId);
    $groupId = $user->getAttribute("group_id");
    return array( "records"=> array( "permissionSetIds"=>$ids, "groupId"=>$groupId ));
  }
