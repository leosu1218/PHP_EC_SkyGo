...
define([
    ...
    'app',
    'configs',
    'text!directives/SbEditor/view.html',
    'jquery'
], function (
    angular,
    app,
    ...
    $)
{
    app.directive("sbEditor", function () {
        return {
            restrict: "E",
            template: view,
            controller: function ($scope, $http, $compile, $timeout) {
                (function( $ ){
                    ...
                    var methods = {
                        ...
                        imageWidget: function(){
                            ...
                            var chooseFromLocal = $('<sb-upload class="col-lg-1" instance="uploader" id="DirectiveUploader"></sb-upload>');
                            ...
                            var menuItems = { 'fonteffects': true,
                                ...
                                'insert_img'	: { "modal": true,
                                    ...
                                    "beforeLoad":function(){
                                        ...
                                        $timeout(function(){
                                            $compile($("#DirectiveUploader"))($scope);
                                        }, 2000)
                                    },
                                    "onSave": function(){
                                        methods.restoreSelection.apply(this);
                                        if($('#imageList').data('current')){
                                            if(navigator.userAgent.match(/MSIE/i)){
                                                var imageStr = '<img src="'+$('#imageList').data('current')+'" height="200" width="200"/>';
                ...
                $scope.$watch("uploader", function(uploader) {
                    if(uploader) {
                        ...
                        $scope.uploader.api(cover_image_api);
                        $scope.uploader.label(cover_image_label);
                        $scope.uploader.mutiple(cover_image_isMutiple);
                        $scope.uploader.success(function (data, status, headers, config) {
                            $scope.productCoverImage = [];
                            var file = data.file;
                            ...
                            var image = $('<img/>', {
                                src: "upload/image/" + file.fileName,
                                title: file.fileName
                ...
                $scope.instance = {
                    getData:function(){
                        return $("#txtEditor").Editor("getText") ;
                    },
